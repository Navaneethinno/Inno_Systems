...
- **Envelope**: every response is `{ message, status, code, remark, data, api }` with `pagination` present on list endpoints only. SYSTEM_API_GUIDE.md says `data` is always an array, but live testing shows that's not true everywhere — **`/system/user/login`'s `data` is a bare object**, not a one-element array. `extractOne()` is defensive and handles both shapes; `extractList()` handles the real list endpoints and the module batch-add. Status casing varies live ("Fail"/"Success", not "FAIL"/"SUCCESS"), so it's compared case-insensitively. A failing status is treated as an error even on HTTP 200.
- `POST /system/user/login` — `Authorization: Basic base64(user:password)` (app credentials, see env above) + JSON body `{ user_name, password }`. `data` (a bare object, confirmed live) holds `user_details`, `user_session_info` (`jwt_token`, `refresh_token`), `full_access`. The institution field is `user_details.institution_name` — confirmed live, not `inst_profile_name` as the guide's sample shows.
- `POST /system/user/refresh_token` — `Authorization: Bearer <refresh_token>`, no body. Response is the session object directly (`jwt_token`/`refresh_token` at the top level), not nested under `user_session_info` like login. Called automatically by the API client on a 401.
...
- `POST /master/{type}/list` — **the one exception without `/system`**, confirmed by curl (the `/system`-prefixed version of `list` returns a generic fallback response, not the real handler). Used both for the 3 writable types above and the read-only reference types (tables at `/reference/:entityKey`).
- `POST /system/user/add` — exactly 5 required fields: `username`, `password_hash`, `profile_id`, `inst_profile_id`, `pwd_policy`. Institution/profile/policy are all name-resolved `<select>`s, not raw ID inputs. Response `data[0]` uses `id`, not `user_id`.
...
None of these dropdown sources are documented in `SYSTEM_API_GUIDE.md`, only confirmed by live requests — worth re-verifying once the backend redeploys.
