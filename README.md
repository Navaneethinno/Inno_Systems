...
- **Envelope**: every response is `{ message, status, code, remark, data, api }` with `pagination` present on list endpoints only. **`data` is always an array**, even for single-record endpoints (login, refresh, one add/edit/delete) — unwrapped via `extractOne()`; multi-record endpoints (lists, the module batch-add) use `extractList()`. Status casing varies live ("Fail"/"Success", not "FAIL"/"SUCCESS"), so it's compared case-insensitively. A failing status is treated as an error even on HTTP 200.
- `POST /system/user/login` — `Authorization: Basic base64(user:password)` (app credentials, see env above) + JSON body `{ user_name, password }`. `data[0]` holds `user_details`, `user_session_info` (`jwt_token`, `refresh_token`), `full_access`. The institution field is `user_details.inst_profile_name`, not `institution_name`.
