...
- `POST /system/institution/add` — dedicated page (`/system/institution`) matching the real nested schema: `language` is `{ default, supported[] }` and `allowed_login_identifiers` is `{ identifiers[] }`, **not the flat comma-separated strings `handoff.md`'s sample implied**. `type` is a numeric code with no documented list of valid values — flagged in the UI.
