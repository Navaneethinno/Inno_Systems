...
- **Pagination** — 10 rows per page with Previous/Next controls, built into `DataTable.jsx` (so it applies everywhere: Master Data, Reference Data, System pages, and the "View all" fullscreen modal). The backend paginates every `/master/{type}` and `/user/*-list`-shaped call at `limit=10` by default — `masterDataService.js`/`systemService.js` request a high limit (1000) so the full dataset is fetched once (this matters for real tables like Countries at 249 rows or Currencies at 156), and `DataTable` slices it into pages of 10 client-side. Where a page fetches with `listWithPagination()` (Master Data, Reference Data), the summary line shows the API's own `pagination.totalRecords` rather than a client-computed count — hidden while a search/filter narrows the visible rows below that total, since the two would otherwise disagree.
