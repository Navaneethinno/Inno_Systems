...
src/
  ...
  features/
    ...
    masterData/
      ...
      services/           # generic list/add/edit/delete for /system/master/*
...
- `/system/master/{module,menu,menu_action}/{add,edit,delete}` — fully documented in the handoff; wired up with real add/edit/delete UI at `/master/:entityKey`.
- `/system/master/{17 other types}/list` — only the list endpoint is documented for these (see `masterEntities.js` for the full set); they get read-only tables at `/reference/:entityKey`. **The list endpoint's HTTP method and response shape aren't documented** — `masterDataService.js` assumes POST and tries a few common envelope shapes; verify against a real response and adjust `extractList` if needed.
