...
- `POST /master/{type}/list` — **no `/system` prefix**, unlike add/edit/delete for the same entities. Used both for the 3 writable types above and the 17 read-only reference types (tables at `/reference/:entityKey`). The list response shape still isn't documented — `masterDataService.js`'s `extractList` tries a few common shapes; adjust if a real response doesn't match.
- `POST /system/profile/add` — nested payload (`profile_info` + `menu_info[]`). Handled by the dedicated `/system/profile` page: pick an institution (via `/institution/profile/get_active`), then check menus and their actions (sourced from the `menu`/`action`/`menu_action` master lists) to build `menu_info`.
- `POST /system/user/add` — institution and profile are `<select>`s sourced from `/institution/profile/get_active` and `/profile/getall` (both `{"view":"dropdown"}`), not raw ID inputs.
